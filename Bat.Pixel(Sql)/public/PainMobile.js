document.addEventListener('DOMContentLoaded', () => {
    ...
    let isTouchMoving = false; // Флаг для перемещения холста при касании экрана
    let currentMode = 'draw'; // Текущий режим: 'draw' или 'move'
    ...
    canvas.addEventListener('touchstart', (event) => {
        ...
        if (currentMode === 'draw' && canDraw()) {
            isTouchDrawing = true;
            drawTouch({ clientX: touch.clientX, clientY: touch.clientY });
    ...
    canvas.addEventListener('touchmove', (event) => {
        ...
        if (isTouchDrawing) {
            ...
            moveCanvas({ clientX: touch.clientX, clientY: touch.clientY });
        }
    });
    ...
    function draw(event) {
        ...
        const x = Math.floor((event.clientX - rect.left - offsetX) / (pixelSize * scale));
        const y = Math.floor((event.clientY - rect.top - offsetY) / (pixelSize * scale));
